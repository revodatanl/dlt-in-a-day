...
-- MAGIC %md Steps:
-- MAGIC - Ingest the data from `/Volumes/databricks_training/raw/baby_names` to the table `baby_names_bronze`. 
...
-- MAGIC - Create a table called `baby_names_silver` and optionally add a comment as well. The table should have three expectations. If the 2nd or 3rd are vialoated, the pipeline should fail. For the first one, it should just flag it.
